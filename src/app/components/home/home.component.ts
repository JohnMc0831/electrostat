...
export class HomeComponent implements OnInit {
  ...
  amIInScope() {
    ...
    const config = {
      ...
      username: 'FAHC\\M212502',
      password: '!2day!2day' // <-- I hexedited the compiled binary and this is properly obfuscated.
    };
    ...
    if (this.alert.affectedGroups) {
      ...
      ad.getGroupMembershipForUser(username, function(err, groups) {
        if (err) {
          that.logger.error(`ERROR: ${JSON.stringify(err)}`);
          return;
        }

        if (!groups) {
          // No groups?!?  How WEIRD!  YOU MUST NOT EXIST!!!
          that.logger.error(`User: ${username} not found!!!`);
        } else {
          that.logger.info(`User Groups: ${JSON.stringify(groups)}`);
        }
        adGrps = groups;
      });

      const grps = this.alert.affectedGroups.split(',');
      grps.forEach(function(value) { // for every group targeted by the alert
        adGrps.forEach(function(val) { // check to see if this user is a member of that group
          if (val.toLowerCase === value.toLowerCase) {
            // report a match.  The alert WILL be shown.
