...
export class HomeComponent implements OnInit {
  ...
  amIInScope() {
    ...
    that.logger.info(`Checking to see if group targeted applies to this user: ${username}.`);
    ...
    ad.findUser('m212502', function(err, usr) {
      ...
      if (usr) {
        if (that.alert.affectedGroups) {
          ...
          grps.forEach(function(grp) { // for every group targeted by the alert
            ad.isUserMemberOf(usr, grp, function(e, isMember) {
              if (err) {
                that.logger.error(`An error occurred determining is ${username} is a member of ${grp}:  ${e}`);
                return;
              }
              that.logger.info(`${username} is a member of ${grp}:  ${isMember}`);
              if (isMember) {
                return true;
              }
            });
